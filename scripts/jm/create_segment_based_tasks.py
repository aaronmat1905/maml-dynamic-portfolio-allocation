"""
Create segment-based tasks to guarantee coverage of all JR2 crisis periods.

This approach creates one task per JR2 segment, ensuring all 8 major crises
are captured (unlike sliding window which only got 2).
...
# Output files
SEGMENT_TASKS_FILE = JM_DIR / "tasks_metadata_segment_based.json"
SEGMENT_SPLIT_FILE = JM_DIR / "task_split_indices_segment_based.json"

# Parameters
MIN_TASK_LENGTH = 30  # Minimum days for a viable task
TARGET_TASK_LENGTH = 40  # Target length for padded tasks
TRAIN_RATIO = 0.52
VAL_RATIO = 0.13
...
def calculate_task_features(regime_df, jump_df, start_date, end_date):
    ...
    # Jump clustering (how close together are jumps?)
    if n_jumps > 1:
    ...
    if n_jumps > 0:
        ...
        negative_jump_ratio = 0.5  # Default
...
def create_segment_based_tasks(segments_df, regime_df, jump_df):
    """Create one task per segment, focusing on JR2 (crisis) segments."""
    print("\n" + "=" * 80)
    print("CREATING SEGMENT-BASED TASKS")
    print("=" * 80)
    ...
    for regime_type in [0, 1, 2]:
        ...
        for idx, segment in regime_segments.iterrows():
            ...
            if duration < MIN_TASK_LENGTH:
                print(f"  ✗ Skipping {start_date.date()} - {end_date.date()} ({duration}d) - too short")
                continue
            
            # Option 1: Use segment as-is if long enough
            ...
            # Option 2: Pad short segments to target length
            else:
            ...
            # Get indices - use get_indexer with nearest method for non-exact dates
            try:
            ...
            # Update task dates to actual dates in index
            ...
            if regime_type == 2:  # Print details for JR2 tasks
...
def split_tasks(tasks):
    """Split tasks into train/val/test chronologically."""
    print("\n" + "=" * 80)
    ...
    for split_name, indices in split.items():
        ...
        regime_counts = {}
        for regime in [0, 1, 2]:
            count = sum(1 for t in split_tasks if t['regime'] == regime)
...
def print_jr2_analysis(tasks):
    """Print detailed analysis of JR2 (crisis) tasks."""
    jr2_tasks = [t for t in tasks if t['regime'] == 2]
    
    print("\n" + "=" * 80)
    print(f"JR2 CRISIS TASKS ANALYSIS ({len(jr2_tasks)} tasks)")
    print("=" * 80)
...
def main():
    ...
    print("=" * 80)
    print("SEGMENT-BASED TASK GENERATION")
    print("=" * 80)
    print(f"\nParameters:")
    print(f"  Min Task Length: {MIN_TASK_LENGTH} days")
    print(f"  Target Task Length: {TARGET_TASK_LENGTH} days")
    ...
    print(f"JR2 Tasks: {sum(1 for t in tasks if t['regime'] == 2)} ✅")
    print(f"\n✓ All 8 JR2 segments converted to tasks!")
    print(f"✓ 2008 Financial Crisis: INCLUDED")
    print(f"✓ Ready for MAML training")
